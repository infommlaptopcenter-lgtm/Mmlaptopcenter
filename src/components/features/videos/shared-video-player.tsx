...
import {
  ...
  type PublicVideo,
  type VideoPlatformValue,
} from "@/lib/video-utils";

const THEMES: Record<
  VideoPlatformValue | "DEFAULT",
  { active: string; hover: string; label: string }
> = {
  DEFAULT: {
    active: "border-[#ea580c] ring-[#ea580c]",
    hover: "hover:border-[#d8a928]",
    label: "text-gray-500",
  },
  YOUTUBE: {
    active: "border-red-600 ring-red-600",
    hover: "hover:border-red-400",
    label: "text-red-600",
  },
  TIKTOK: {
    active: "border-cyan-400 ring-pink-500",
    hover: "hover:border-cyan-400",
    label: "text-pink-500",
  },
  FACEBOOK: {
    active: "border-blue-600 ring-blue-600",
    hover: "hover:border-blue-400",
    label: "text-blue-600",
  },
  INSTAGRAM: {
    active: "border-fuchsia-500 ring-orange-400",
    ...
    <div className="grid min-w-0 gap-5 overflow-hidden lg:grid-cols-[minmax(0,1fr)_minmax(260px,340px)]">
      <div className="min-w-0 overflow-hidden rounded-2xl border border-[#d8a928]/20 bg-white shadow-sm">
        <div className="flex aspect-video w-full items-center justify-center overflow-hidden bg-[#090909]">
          ...
          >
            {selected.embedUrl && playingId === selected.id ? (
              <iframe
                key={selected.id}
              ...
              >
                ...
                <span className="relative z-10 flex h-16 w-16 items-center justify-center rounded-full bg-red-600 text-white shadow-2xl transition group-hover:scale-110 group-focus-visible:ring-4 group-focus-visible:ring-white">
              ...
              <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-6 text-center text-white">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-bold text-gray-950"
                >
                  Open on {VIDEO_PLATFORM_LABELS[selected.platform]}{" "}
        ...
        <div className="space-y-2 p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3">
            <h3 className="font-serif text-xl font-bold leading-tight text-gray-950">
              {selected.title}
            </h3>
          </div>
      ...
      <aside className="min-w-0 overflow-hidden">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 lg:max-h-[520px] lg:flex-col lg:overflow-x-hidden lg:overflow-y-auto lg:pr-1">
          {videos.map((video) => (
            ...
            >
              <span
                className={`relative flex shrink-0 items-center justify-center overflow-hidden rounded-lg bg-black ${video.format === "VERTICAL" ? "h-24 w-[54px]" : "h-[72px] w-32"}`}
              >
                {video.thumbnail ? (
                  <Image
                    src={video.thumbnail}
              ...
              <span className="min-w-0">
                <span className="line-clamp-2 text-sm font-bold leading-snug text-gray-950">
                  {video.title}
                </span>
                <span
                  className={`mt-1 block text-xs font-semibold ${styles.label}`}
                >
