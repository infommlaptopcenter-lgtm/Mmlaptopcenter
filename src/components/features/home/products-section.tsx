...
export function NewArrivalsSection({
  ...
  return (
    <section className="mx-auto w-full max-w-7xl overflow-hidden bg-white px-6 py-16 lg:px-8">
      ...
      >
        {(newArrivals.length > 1 ? [0, 1] : [0]).map((copy) =>
          newArrivals.map((product) => {
            const imageUrls = Array.isArray(product.images)
              ? product.images.filter((image): image is string => typeof image === "string")
              : [];
            const validImages = [product.featuredImage, ...imageUrls]
              .filter((img): img is string => typeof img === "string" && img.trim().length > 0)
              .filter((img) => !failedImages.includes(img));
            const activeImage = validImages[0] || FALLBACK_IMAGE;
            ...
            return (
              ...
              >
                <Image
                  ...
                  onError={() => {
                    if (activeImage && activeImage !== FALLBACK_IMAGE && !failedImages.includes(activeImage)) {
                      setFailedImages((prev) => [...prev, activeImage]);
                    }
                  }}
                />
          ...
          {[0, 1].map((copy) => (
            ...
            >
              {mainCategories.map((category) => {
                const categoryImg =
                  category.image && !failedImages.includes(category.image)
                    ? category.image
                    : FALLBACK_IMAGE;
                    ...
                    <div className="relative h-32 w-32 overflow-hidden rounded-2xl sm:h-40 sm:w-40 lg:h-48 lg:w-48">
                      <Image
                        ...
                        onError={() => {
                          if (
                            category.image &&
                            !failedImages.includes(category.image)
                          ) {
                            setFailedImages((prev) => [...prev, category.image!]);
                          }
                        }}
                      />
                    </div>
...
function FeaturedProductRow({
  ...
  const displayProducts = isScrollable ? [...products, ...products] : products;
    ...
    <div className="relative">
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 transition-opacity duration-1000 ease-in-out">
        {displayCollections.map((collection, i) => {
          const collectionImg =
            collection.image && !failedImages.includes(collection.image)
              ? collection.image
              : FALLBACK_IMAGE;
              ...
              <div className="relative aspect-[16/10]">
                <Image
                  ...
                  onError={() => {
                    if (
                      collection.image &&
                      !failedImages.includes(collection.image)
                    ) {
                      setFailedImages((prev) => [...prev, collection.image!]);
                    }
                  }}
                />
              </div>
