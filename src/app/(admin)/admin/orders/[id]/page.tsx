...
export default function OrderDetailPage() {
  ...
  if (state.error && !state.order) return <ErrorView error={state.error} />;
  if (!state.order) return null;

  return (
    <div className="p-4 sm:p-6 xl:p-8">
      <OrderHeader order={state.order} />
      {state.error ? <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{state.error}</div> : null}
      {state.notice ? <div className="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-700">{state.notice}</div> : null}
      ...
      <div className="grid items-start gap-6 xl:grid-cols-2">
        <div className="min-w-0 space-y-6">
          ...
          <OrderCustomerPanel order={state.order} sendingWhatsApp={state.sendingWhatsApp} sendingEmail={state.sendingEmail} openCustomerWhatsApp={state.openCustomerWhatsApp} sendOrderConfirmation={state.sendOrderConfirmation} sendConfirmationEmail={state.sendConfirmationEmail} />
        </div>
        <div className="min-w-0 space-y-6">
