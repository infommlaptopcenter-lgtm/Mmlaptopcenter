...
export function OrderStatusPanel({
  ...
  return (
    <div className="rounded-xl border border-[#d8a928]/20 bg-white p-4 shadow-sm sm:p-5">
      <div><p className="text-xs font-bold uppercase tracking-wider text-[#c86f2d]">Fulfillment</p><h2 className="mt-1 text-base font-bold text-[#0a0a0a]">Manage order</h2></div>
      <div className="mt-4 grid gap-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2"><Input label="Courier / delivery partner" value={form.courierName} onChange={(value) => setForm((f) => ({ ...f, courierName: value }))} placeholder="TCS, Leopards, Trax..." /><Input label="Tracking ID" value={form.trackingNumber} onChange={(value) => setForm((f) => ({ ...f, trackingNumber: value }))} placeholder="Shipment number" /></div>
        <Input label="Tracking URL" value={form.trackingUrl} onChange={(value) => setForm((f) => ({ ...f, trackingUrl: value }))} />
        <Input label="Estimated delivery" type="date" value={form.estimatedDelivery} onChange={(value) => setForm((f) => ({ ...f, estimatedDelivery: value }))} />
...
function Input({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string }) {
  return <label className="block text-sm font-medium text-[#0a0a0a]">{label}<input type={type} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-transparent focus:ring-2 focus:ring-[#d8a928]" /></label>;
}
